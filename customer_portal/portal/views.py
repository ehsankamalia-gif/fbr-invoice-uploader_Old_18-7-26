...
def custom_admin_spare_ledger_monthly_report_view(request):
    ...
    for tx in transactions:
        ...
        if tx.trans_type == "CREDIT":
            ...
            if is_bank:
                month_stats[month_key]["bank_debit"] += amount
            else:
                month_stats[month_key]["hard_cash_debit"] += amount
    ...
    for month_key in sorted_months:
        ...
        hard_cash_credit = stats["hard_cash_credit"]
        bank_debit = stats["bank_debit"]
        hard_cash_debit = stats["hard_cash_debit"]

        total_in = bank_credit + hard_cash_credit
        total_out = bank_debit + hard_cash_debit

        closing_balance = running_bf + total_in - total_out

        report_data.append({
            "month_key": month_key,
            "brought_forward": running_bf,
            "bank_credit": bank_credit,
            "hard_cash_credit": hard_cash_credit,
            "total_credit": total_in,
            "bank_debit": bank_debit,
            "hard_cash_debit": hard_cash_debit,
    ...
    grand_total_in = sum(row["total_credit"] for row in report_data)
    grand_total_out = sum(row["total_debit"] for row in report_data)
    overall_balance = running_bf

    context = {
        "title": "Spare Parts Ledger - Monthly Report",
